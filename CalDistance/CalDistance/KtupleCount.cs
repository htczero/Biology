...
namespace CalDistance
{
    class KtupleCount
    {
        ...
        public KtupleCount(string filePath,int k)
        {
            GetSeqText(filePath);
            this.k = k;
        }
        ...
        {           
            ...
            {
                if (listSeq[i].Length >= k)
                {
                    total += (listSeq[i].Length - k + 1);
        ...
        private void GetKtupleCount(int k, string seqText, int[] arrKtuple)
        {
            int length = seqText.Length - k + 1;

            int key = 0;
            for (int i = 0; i < k; i++)
            {
                int tmp = ConvertDate(seqText[i]);
                for (int j = k - 1; j > i; j--)
                {
                    tmp *= 4;
                }
            ...
            {
                int head = ConvertDate(seqText[i - 1]);
                key = (key - head * flag) * 4 + ConvertDate(seqText[i + k - 1]);
                arrKtuple[key]++;
            }
        }
