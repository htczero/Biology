...
namespace d2_toolsPro
{
    class DissimiliratyS2 : Dissimiliraty
    {
        ...
        public DissimiliratyS2(Dictionary<string, string> dic, string savePath, List<int> list, string str, int precision = 5) : base(str)
        {
        ...
        private void CalDissimiliratyMatrix(int k)
        {
            ...
            foreach (var j in listR)
            {
                ...
                dicResultS2[j].Add(Math.Round(resultS2, precision).ToString());
            }//for
        }
        ...
        private void GetDissimiliratyMatrixForOneRow(int k)
        {
            ...
            {
                WriteFile("S2", item.Value, "_M" + item.Key, k);
            }
